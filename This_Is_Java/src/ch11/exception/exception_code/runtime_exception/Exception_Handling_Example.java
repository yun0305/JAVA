...
public class Exception_Handling_Example {
	...
	try {
		...
		System.out.println(e.getMessage());//예외객체 e로 가서 예외객체가 가지고 있는 getMessage()메소드를 호출하면 
										   //왜 에러가 뜨는지 내용을 얻을수 있다.
		...
		일반예외는 컴파일러가 어떤 코드가 예외 처리가 필요한지 알려준다.
		하지만 실행예외는 어떤 부분에서 예외가 발생할지 실행되기 전까지는 모름
		*/
