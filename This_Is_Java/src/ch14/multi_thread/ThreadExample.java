...
public class ThreadExample {
	 ...
	 *  스레드는 코드의 실행 흐름을 말하는데, 프로세스 내에 스레드가 두 개라면 두개의 코드 실행 흐름이
	 *  생긴다는 의미다.
	 *  
	 *  멀티 프로세스가 프로그램 단위의 멀티 태스킹이라면 멀티 스레드는 프로그램 내부에서의 멀티 태스킹
	 *  이라고 볼 수 있다.
	 *  
	 *  멀티 프로세스들은 서로 독립적이므로 하나의 프로세스에서 오류가 발생해도 다른 프로세스에서 영향을
	 *  미치지 않는다.
	 *  
	 *  하지만 멀티 스레드는 프로세스 내부에서 생성되기 때문에 하나의 스레드가 예외를 발생시키면
	 ...
	 *  안드로이드 앱에서 네트워크 통신을 하기 위해 사용하기도 한다.
	 *  
	 *  또한 다수의 클라이언트 요청을 처리하는 서버를 개발할 때에도 사용된다.
	 *  
	 *  프로그램 개발에 있어서 멀티 스래드는 꼭 필요한 기능이기 때문에 반드시 이해하고
	 *  활용할 수 있도록 한다.
	 */
}
