...
public class A {
	...
	/*
	 * 정적 내부 클래스를 사용하면 이클래스는 특정 외부 클래스와 강하게 연관된 개념이다 라는 의미를 줄수 있다.
	 * 즉 외부 클래스의 일부로 취급하면서도 독립적으로 사용할 수 있는 구조를 만들고 싶을 떄 유용하다.
	 */
	...
	static class B{// 정적 키워드가 붙으면 외부에서 A객체 없이도 A.B로 접근 가능
		 ...
		 void method1B() {
			 //a = 10; 내부 클래스가 정적이라 외부의 인스턴스에는 직접 접근할수 없다
			 // 접근하려면 객체가 필요함
			 //그말은 즉 내부 클래스가 정적이면 외부클래스의 인스턴스 멤버는에는 접근할수 없다(객체 없이는)
			 A test = new A();//이 방법은 사실 권장하지 않는다 메모리가 낭비될 가능성이 있다.
			 test.a = 10;
