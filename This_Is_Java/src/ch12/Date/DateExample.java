 ...
 * Date 클래스에는 여러 개의 생성자가 선언되어 있지만 대부분 Deprecated(더이상 사용되지 않음)되어Date()
 * 생성자만 주로 사용된다. Date 생성자는 컴퓨터의 현재 날짜를 읽어 Date 객체로 만든다.
 * 
 * Date now = new Date();
 * 
 * 현재 날짜를 문자열로 얻고 싶다면 toString() 메소드를 사용할 수 있지만
 * 영문으로 출력되기 때문에 우리가 원하는 형식이 아니다.
 * 
 * 원하는 문자열로 얻고 싶다면 
 * SimpleDateFormat 클래스와 함께 사용해야 한다. 다음 예제는 "년 월 일 시 분 초" 형식으로
 * 문자열을 얻는 방법을 보여준다.
 * 
...
public class DateExample {
	...
	public static void main(String[] args) {
		...
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy.MM.dd hh:mm:ss");//대소문자 의미 있음
