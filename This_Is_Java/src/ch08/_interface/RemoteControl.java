...
public interface RemoteControl{
	...
	//추상메소드 선언
	public void turnOn();
	//인터페이스에서 추상메소드는 기본이 public 이다.
	//그리고 abstract는 생략이 가능하다.
	/*Remortcontol로 turnOn 메소드를 사용할 수 있게 해준다. 실제 작동하지는 않고
	  
	  구현 클래스에서 작동된다.
	  구현 클래스는 인터페이스가 가지고 있는 기능을 그대로 가지고 있어야 한다.
	  
      구현 객체는 인터페이스를 implements 받으면 인터페이스가 가지고 있는 추상 메소드를 받아와서
	  재정의 한 다음에 사용한다.
	  
	  인터페이스에 선언된 추상 메소드는 "모두" 구현객체가 가지고 있어야 한다.
